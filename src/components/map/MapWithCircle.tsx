...
export const MapWithCircle = ({ 
  ...
  }, [area, latitude, longitude, showCircle, circleRadius]);

  const getCircleColor = () => {
    if (!trafficLevel) return "#3B82F6"; // blue default
  ...
  const getTrafficColor = () => {
    if (!trafficLevel) return "bg-blue-50 border-blue-200";
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {!mapLoaded || mapError ? (
          <div
            className="h-96 bg-muted/50 cursor-pointer group relative"
      ...
      <div className="mt-3 text-xs text-muted-foreground text-center">
        Interactive map powered by Google Maps{showCircle && " • Circle indicates monitoring area"} • {mapLoaded && !mapError ? "Live traffic layer enabled" : "Click to explore"}
      </div>
    </div>
  );
};
